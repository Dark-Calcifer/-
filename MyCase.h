...
inline T MyCase<T>::operator[](int index)
{
	if (index > (arrSize - 1))
	{
		throw exception("ѕопытка обращени€ к неинициализированной пам€ти");
	}
	else
	{
	...
	case 1: // name
	{
		for (int i = 0; i < arrSize - 1; i++)
		{
			for (int j = 0; j < arrSize - 1; j++)
			{
				...
				if (strcmp(temp1, temp2) > 0) // strcmp если 1 > 2, возвращает число больше 0, если меньше - меньше нул€, если равно 0
				{
					temparr = arr[j];
					arr[j] = arr[j + 1];
					arr[j + 1] = temparr;
